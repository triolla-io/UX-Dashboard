...
describe('POST /api/feedback — validation', () => {
  it('returns 400 when image is missing', async () => {
    const res = await request(app).post('/api/feedback').set('x-forwarded-for', nextIp()).send({ mediaType: 'image/png' })
    expect(res.status).toBe(400)
    expect(res.body.error).toBe('image is required')
  })

  it('returns 400 when mediaType is invalid', async () => {
    const res = await request(app).post('/api/feedback').set('x-forwarded-for', nextIp()).send({ image: 'abc123', mediaType: 'image/gif' })
    expect(res.status).toBe(400)
  ...
  it('returns 400 when mediaType is missing', async () => {
    const res = await request(app).post('/api/feedback').set('x-forwarded-for', nextIp()).send({ image: 'abc123' })
    expect(res.status).toBe(400)
    expect(res.body.error).toBe('mediaType must be image/png, image/jpeg, or image/webp')
  })
  ...
  it('accepts image/jpeg as a valid mediaType (not a 400)', async () => {
    ...
    const res = await request(app).post('/api/feedback').set('x-forwarded-for', nextIp()).send({ image: 'abc', mediaType: 'image/jpeg' })
...
describe('POST /api/feedback — structured audit', () => {
  ...
  it('returns a validated AuditResult with code-computed overall and verdict', async () => {
    ...
    const res = await request(app).post('/api/feedback').set('x-forwarded-for', nextIp()).send({ image: 'abc', mediaType: 'image/png' })
  ...
  it('feeds the DDIA knowledge into the system prompt', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(okModelResponse(MODEL_JSON))
    await request(app).post('/api/feedback').set('x-forwarded-for', nextIp()).send({ image: 'abc', mediaType: 'image/png' })
  ...
  it('truncates context to 200 chars before sending to the model', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(okModelResponse(MODEL_JSON))
    await request(app).post('/api/feedback').set('x-forwarded-for', nextIp()).send({ image: 'abc', mediaType: 'image/png', context: 'x'.repeat(300) })
    const body = JSON.parse((vi.mocked(fetch).mock.calls[0][1] as RequestInit).body as string)
...
describe('POST /api/feedback — IP rate limiting', () => {
  ...
  it('returns 429 after 2 requests from the same IP', async () => {
    ...
    await request(app).post('/api/feedback').set('x-forwarded-for', '5.5.5.5').send({ image: 'abc', mediaType: 'image/png' })
    await request(app).post('/api/feedback').set('x-forwarded-for', '5.5.5.5').send({ image: 'abc', mediaType: 'image/png' })

    const res = await request(app).post('/api/feedback').set('x-forwarded-for', '5.5.5.5').send({ image: 'abc', mediaType: 'image/png' })
